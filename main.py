	...
	for guild in client.guilds:
		...
		for game in game_title_list:
			...
			if game not in algamelist:
				log("---- 作成")
				guilddata[str(
					guild.id)]["gamelist"][game] = default_gamelist_item
...
async def on_ready():
	...
	await client.change_presence(activity=discord.Game(
		name=f"/help | Version {bot_version}"))
...
class InviteView(discord.ui.View):
	...
	@discord.ui.select(
		...
		max_values=1,
		options=[
			discord.SelectOption(label="Rainbow Six Siege",
								 description="Ubisoft Montréal"),
			discord.SelectOption(label="Apex Legends",
	...
	async def button_callback(self, button, interaction):
		...
		async def updateMemberList():
			...
			for field in original_embed.fields:
				if field.name.startswith(":busts_in_silhouette: 参加者") is True:
					...
					field.value = convertToUserBulletPointsFromIDList(
						invitedata[id]["member"])
			await rmsg.edit(rmsg.content,
							embed=original_embed,
							view=InviteView(timeout=invitedata[id]["timeout"]))

		async def sendJoinMessage():
			# 埋め込みメッセージを作成して返信
			embed = discord.Embed(color=discord.Colour.from_rgb(131, 177, 88))
			embed.set_author(name=f"{interaction.user} さんが参加しました",
		...
		# それ以外の場合
			...
			# 募集に参加していない場合
				...
				if len(invitedata[id]
					   ["member"]) + 1 >= invitedata[id]['nop'] + 1:
					# 募集データにユーザーIDを追加
...
async def recruitment(
	...
	if ud["Atmark"] == True:
		...
		embed.add_field(name=f":no_entry_sign: 既に募集が開始されています！",
						value=f"再度募集を行うには、一度募集をキャンセルしてください！")
		...
		embed.add_field(name=f":busts_in_silhouette: 参加者 (1/{nop + 1})",
		...
		notification_embed = discord.Embed(
			...
			description=f"[クリックで募集メッセージへ]({rmsg.jump_url})")
...
async def setrole(ctx, game: Option(str,
									name="ゲーム",
									description="設定するゲームタイトル",
									choices=dict.keys(gamelist)),
				  role: Option(discord.Role,
							   name="ロール",
							   description="募集時にメンションするロール")):
	global guilddata
...
@client.command(description="メンバー募集メッセージを送信するテキストチャンネルを設定します。",
				permission=discord.Permissions.administrator)
async def setrecruitmentchannel(ctx, ch: Option(
	discord.TextChannel,
	name="テキストチャンネル",
	description="メンバー募集のメッセージを送信するテキストチャンネル")):
	global guilddata
...
@client.command(
	description=
	"現在設定されているメンバー募集メッセージを送信するテキストチャンネルを削除します。削除した場合、メンバー募集メッセージはコマンドを実行したチャンネルへ送信されます。",
	permission=discord.Permissions.administrator)
...
async def help(ctx):
	...
	embed.set_footer(
		text=
		f"{bot_name} Bot - Version {bot_version}\nDeveloped by Milkeyyy#0625")
	await ctx.respond(embed=embed, ephemeral=True)
